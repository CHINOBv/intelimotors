...
export class AdService {
  ...
  async publishAd(
    ...
    const publishService: IPublishService =
      PublishServiceFactory.createService(site);
    ...
    try {
      screenshotPath = await publishService.createAd(
        { ...adDetails, images: imagePaths },
        ""
      );
    } catch (error) {
    ...
    const adId = ad.id;
    const screenshotResultPath = path.join(
      __dirname,
      "../../../uploads",
      userId,
      "ads",
    ...
    try {
      ...
      throw new Error(
        `Failed to move screenshot from ${screenshotPath} to ${screenshotResultPath}: ${error}`
      );
    }

    const adDir = path.join(
      __dirname,
      "../../../uploads",
      userId,
      "ads",
    ...
    const images = movedImagePaths.map(
      (imagePath) =>
        new Image(
          path.relative(path.join(__dirname, "../../../"), imagePath),
          ad
        )
    );
